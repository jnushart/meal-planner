...
function getAccountEmail() {
  return String(currentUser?.email || '').trim().replace(/[\r\n]/g, '');
}
function bindAccountEmail() {
  const email = getAccountEmail();
  const input = $('emailRecipient');
  if (email && input) {
    input.value = email;
    localStorage.setItem(STORAGE.email, JSON.stringify(email));
  }
  return email;
}
async function handleAuthSession(session, authEvent = '') {
  ...
  try {
    ...
    bindAccountEmail();
    if (authEvent === 'PASSWORD_RECOVERY' || passwordRecoveryRequested) {
...
async function requestPasswordReset() {
  ...
  status.textContent = 'Check your email. The link will let you choose a password.';
}
async function updatePassword(event) {
  ...
  status.classList.remove('error');
  if (password.length < 6) { status.textContent = 'Use at least 6 characters.'; status.classList.add('error'); return; }
  if (password !== confirmation) { status.textContent = 'Those passwords do not match.'; status.classList.add('error'); return; }
  if (!supabaseClient || !currentUser) { status.textContent = 'Your password link has expired. Request a new one and try again.'; status.classList.add('error'); return; }
...
async function initializeCloudAuth() {
  ...
  passwordRecoveryRequested = window.location.hash.includes('type=recovery');
...
function downloadEmailEml(message = '') {
  const recipient = bindAccountEmail();
...
async function sendWeekEmail() {
  ...
  if (!recipient) {
    status.textContent = 'Your account email was not available. Sign in again and try once more.';
